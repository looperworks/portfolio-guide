...
const DIAGRAM_MAP = {
  ...
  8: [
    { image: "class-pdf/class4-grid-origins.png", title: "The Grid Before Graphic Design", alt: "Dürer perspective grid woodcut, Alexandria city plan, golden section floor plan analysis, and Japanese Ken module interior", caption: "The grid is not a modern invention. Dürer's perspective device (1525) imposed a coordinate system on observed reality. The Hippodamian plan of Alexandria organized an entire city along orthogonal axes. The golden section governed spatial proportion in classical architecture. The Ken module gave Japanese construction a repeating structural unit that organized rooms, corridors, and gardens from a single dimension. Every portfolio grid inherits this lineage: a system of measurement that turns space into structure." },
    { image: "class-pdf/class4-architectural-grids.png", title: "The Grid Across Architectural History", alt: "Nine plan analyses from Temple of Solomon through Kersten Geers Villa, with structural grids overlaid in red", caption: "Nine buildings, five centuries, one principle. Villalpando's Temple of Solomon (1596) uses a hierarchical grid of nested courts. Palladio's Villa Capra (1565) rotates a nine-square around a central void. Le Corbusier's Villa Savoye (1931) liberates the grid from the wall. Hejduk's House I (1954) and Eisenman's House III (1971) treat the grid as the subject of the architecture itself. Kahn, OMA, and Geers continue the investigation. The red overlays make explicit what the buildings already know: the grid is not applied to the design. The grid is the design." },
    { image: "class-pdf/class4-palazzo-diagrams.png", title: "Grid Logic in a Real Building", alt: "Palazzo facade analysis showing column bays, loggia rhythm, stair displacement, and window patterns across four diagrams", caption: "A palazzo facade read as a grid system. Column bays establish the primary rhythm. Window patterns repeat within that rhythm. When the stair displaces one bay, the entire system adjusts: the loggia shifts, the symmetry rebalances, and the grid accommodates the exception without breaking. This is exactly what a portfolio grid must do: maintain structure while absorbing variation. The diagrams read like a spread analysis: dominant element, subordinate elements, alignment logic, and the one intentional break." },
    { image: "class-pdf/class4-facade-structure.png", title: "The Design Analogy: Facade and Structure", alt: "Concrete facade split diagonally to reveal underlying grid: primary load-bearing frame, secondary alignment grid, content containers, typographic baseline, and margin guides", caption: "The facade is what the reviewer sees. The grid is what holds it together. This image splits a concrete building diagonally to reveal the structural system beneath: primary load-bearing frame (the column grid), secondary alignment (cyan), content containers (magenta), typographic baseline, and margin guides. A portfolio page works the same way. The reader sees images, text, and whitespace. Beneath that surface sits the modular grid, the baseline grid, and the margin system that make every placement deliberate." },
    { image: "class-pdf/class4-four-grid-types.png", title: "Four Grid Types in Practice", alt: "Manuscript grid in a novel, multi-column grid in a bible, modular grid in an architecture brochure, and hierarchical grid on a web portfolio", caption: "Four documents, four grid structures. The novel uses a manuscript grid: one text block, generous margins, nothing else. The bible uses a multi-column grid: two or three columns per page, optimized for dense text and cross-referencing. The architecture brochure uses a modular grid: rows and columns create discrete zones for images, text blocks, and captions. The web portfolio uses a hierarchical grid: content arranged by visual weight rather than geometric rule. For architectural portfolios, the modular grid is the recommended structure because it handles the widest range of content: plans, sections, renderings, models, and text within a single repeating framework." },
    { image: "class-pdf/class4-baseline-vs-modular.png", title: "Two Systems, One Page", alt: "Baseline grid with horizontal cyan lines for text precision alongside modular grid with rectangular content zones", caption: "The baseline grid and the modular grid serve different purposes on the same page. The baseline grid (left) locks every line of text to a consistent vertical increment, ensuring columns align across the spread. It governs precision. The modular grid (right) divides the page into rectangular zones that organize images, sidebars, and mixed content. It governs placement. A professional portfolio uses both: the modular grid positions elements, the baseline grid aligns the text within them. Neither is optional." },
  ],
  9: [
